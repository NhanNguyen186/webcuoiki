<?php
    ...
    $get_page = isset($_GET['id'])?$_GET['id']:1;
    $new_blog->n_category_id = $get_page;
    @$result = $new_blog->read_single_category();

    $new_cate = new category($db);
    $new_cate->n_category_id = $get_page;
    @$new_cate->read_single();
        ...
        <div class="row">
            <div class="col-12">
                <div class="page-header flex justify-content-center align-items-center" style="background-image: url('images/toty.jpg')">
                    <h1>Category<?php echo $new_cate->v_category_title; ?></h1>
                </div><!-- .page-header -->
            </div><!-- .col -->
        </div><!-- .row -->
    </div><!-- .hero-section -->
